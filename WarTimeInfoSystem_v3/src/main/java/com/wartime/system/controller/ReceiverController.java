...
public class ReceiverController {
    ...
    private void refreshMessages() {
        ...
        var messages = MessageService.getInstance().getAllMessages();
        if (messages.isEmpty()) {
            lblStatus.setText("NO OPERATIONAL INSTRUCTIONS PASSED YET");
            lstMessages.setItems(FXCollections.emptyObservableList());
        } else {
            lblStatus.setText("Operational Messages:");
            lstMessages.setItems(FXCollections.observableArrayList(messages));
        }
    }
        ...
        result.ifPresent(pair -> {
            ...
            try {
                ...
                if (accessGranted) {
                    ...
                    dialogContent.getDialogPane().setStyle("-fx-background-color: #2b2b2b;"); // Match theme
    ...
    private boolean checkAccess(SecureMessage msg, String statedAppointment) {
        com.wartime.system.model.Rank userRank;
        try {
            userRank = com.wartime.system.model.Rank.valueOf(statedAppointment.toUpperCase());
        } catch (IllegalArgumentException e) {
            throw new RuntimeException("Invalid Appointment Type");
        }

        com.wartime.system.model.Rank targetRank = msg.getTargetRank();

        // Safety check if targetRank was not set in old messages (legacy support)
        if (targetRank == null) {
            // Default: if sender was Commander, restricted to Commander?
            // Or fallback to sender rank?
            targetRank = msg.getSenderRank();
        }

        // Logic: Rank is Enum (COMMANDER=0, OFFICER=1, SOLDIER=2)
        // Access if UserRank <= TargetRank (Lower ordinal means Higher/Equal Rank)
        // e.g. Target=OFFICER(1).
        // Commander(0) <= 1 OK.
        // Officer(1) <= 1 OK.
        // Soldier(2) <= 1 False. Denied.
