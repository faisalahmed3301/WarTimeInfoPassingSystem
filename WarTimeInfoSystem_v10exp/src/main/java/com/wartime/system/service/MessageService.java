...
import com.wartime.system.exception.SessionException;
import com.wartime.system.exception.InvalidRankException;
import com.wartime.system.exception.InsufficientRankException;
import com.wartime.system.exception.UnknownStrategyException;
import com.wartime.system.exception.InvalidDecryptionKeyException;
import com.wartime.system.exception.MessageTargetException;
...
public class MessageService {
    ...
    public void sendMessage(String plainText, String key, String strategyName, String appointment, Group group,
            AbstractUser targetUser, String priority) {
        ...
        if (appointment != null) {
            try {
                ...
                throw new InvalidRankException("Invalid Appointment Rank: " + appointment, e);
            }
        } else if (group == null && targetUser == null) {
            throw new MessageTargetException("Either Appointment, Group, or Target User must be specified.");
        }
    ...
    public void sendEmergencyMessage(String plainText) {
        ...
        if (currentUser.getRank() != Rank.COMMANDER)
            throw new InsufficientRankException("Only Commanders can send emergency alerts");
    ...
    private EncryptionStrategy resolveStrategy(String name) {
        ...
        switch (name) {
            ...
            default:
                throw new UnknownStrategyException("Unknown Strategy: " + name);
        }
    }
    ...
    public List<SecureMessage> getVisibleMessages(AbstractUser user) {
        return messages.stream()
                .filter(m -> {
                    ...
                    // If it's a private individual chat — target user sees it
                    if (m.getTargetUser() != null) {
                        return m.getTargetUser().getName().equalsIgnoreCase(user.getName());
                    }

                    // If it's a group message — only group members see it
                    if (m.getTargetGroup() != null) {
                        return m.getTargetGroup().isMember(user);
                    }

                    // If it's a rank-wide broadcast — hierarchy enforced
    ...
    public String attemptDecryption(SecureMessage message, String inputKey) {
        if (!message.getEncryptionKey().equals(inputKey)) {
            throw new InvalidDecryptionKeyException("Invalid Decryption Key");
        }
